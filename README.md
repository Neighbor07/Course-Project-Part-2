...
Log into your AWS Academy dashboard and start the lab.

Click the AWS Details button, then select Show next to the AWS CLI label.

Copy the entire text block containing the access keys.

Open your WSL Ubuntu terminal and execute the following commands to save the keys securely:

Bash
mkdir -p ~/.aws
nano ~/.aws/credentials
Paste the copied credentials text directly into the text editor file.

Save the modifications by pressing Ctrl+O, press Enter to confirm the filename, and exit the text editor interface by pressing Ctrl+X.
...
Audit the public visibility of your cloud-hosted Minecraft game instance engine using the target network port scanner utility, replacing the placeholder text with your actual server IP address:

Bash
nmap -sV -Pn -p T:25565 <YOUR_SERVER_PUBLIC_IP> - generated
...
Linux Operating System Process Lifecycles: Freedesktop.org. (2026). systemd.service: Core Background Process Service Unit Configurations. Linux Manual Pages System. https://www.freedesktop.org/software/systemd/man/latest/systemd.service.html?__goaway_challenge=meta-refresh&__goaway_id=3054d3ba1cb5644f24017d8b4f2b13b9&__goaway_referer=https%3A%2F%2Fgemini.google.com%2F
